...
class DateTime extends \DateTime
{
  ...
  function get_day_longname($date="") {
	...
	// Return something like: 0 for Sunday, 1 for Monday ...until 6 for Saturday
  function get_wday($date="") {
  	if ($date=="") {
  		$date = $this->mdate();
 		}
		return getdate(strtotime($date))["wday"];
 	}
	...
	function isoDate($fromFormat, $date) {
	...
	function isoDateTime($fromFormat, $datetime) {
	...
	function getYear($datetime) {
		return (new \DateTime($datetime))->format('Y');
	}
	function getMonth($datetime) {
		return (new \DateTime($datetime))->format('m');
	}
	function getDay($datetime) {
		return (new \DateTime($datetime))->format('d');
	}
	function getHour($datetime) {
		return (new \DateTime($datetime))->format('H');
	}
	function getMinute($datetime) {
		return (new \DateTime($datetime))->format('i');
	}
	function getSecond($datetime) {
		return (new \DateTime($datetime))->format('s');
	}
}